    ...
    PositionEmbeddingType const position_embedding_type)
{
    ...
    if (valid_seq)
    {
        mmha::rotary_initialize(rotary_embedding_base, rotary_embedding_scale, rotary_scale_type,
            ...
            yarn_low, yarn_high, rotary_embedding_yarn_mscale, rotary_embedding_yarn_beta_fast,
            rotary_embedding_yarn_beta_slow, rotary_embedding_yarn_attn_factor);
    }
...
#pragma unroll
    ...
    case PositionEmbeddingType::kROPE_GPTJ:
    {
        mmha::apply_rotary_embedding(
            q, k, tidx, rotary_embedding_dim, rotary_embedding_base, rotary_embedding_scale,
            dst_kv_seq_idx, rotary_scale_type, rotary_embedding_max_positions, yarn_low, yarn_high,
...
void invokeApplyBiasRopeUpdateKVCacheDispatch(T* QKV, KVCacheBuffer& kvTable, const T* qkv_bias, const int* seq_lens,
    ...
    const float rotary_embedding_yarn_beta_slow, const PositionEmbeddingType position_embedding_type, const float* scale,
    ...
    const int int8_mode, const KvCacheDataType cache_type, const float* kvScaleOrigQuant, cudaStream_t stream)
{
    ...
    if (cache_type == KvCacheDataType::INT8)
    {
        invokeApplyBiasRopeUpdateKVCacheDispatch<T, int8_t, KVCacheBuffer>(QKV, kvTable, qkv_bias, seq_lens,
            ...
            rotary_embedding_yarn_extrapolation_factor, rotary_embedding_yarn_attn_factor, rotary_embedding_yarn_beta_fast,
            rotary_embedding_yarn_beta_slow, position_embedding_type, scale, kvScaleOrigQuant, int8_mode, stream);
    }
#ifdef ENABLE_FP8
